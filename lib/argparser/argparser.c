...
static struct option option_to_gnu(const Option option) {
  const struct option gnu_option = {option.long_name, no_argument, NULL,
                                    option.short_name};
  return gnu_option;
}
...
static int argparser_getopt(const int argc,
  ...
  for (int index = 0; index < option_count; index++) {
    ...
    if (optstring == NULL) {
      ...
      hx_strcat(optstring, sizeof(optstring), &argparser_option.short_name,
                SEQUENTIAL);
    }
  }
  free(gnu_options);
...
void describe(const char* name,
  ...
  description_init(description, name, overview, option_count, options,
                   subcommand_count, subcommands);
...
DefaultExecutionStatus execute_default(const int argc, char** argv) {
  int flag;
  while ((flag = argparser_getopt(argc, argv, default_option_count,
                                  default_options)) != -1) {
    for (int option_index = 0; option_index < default_option_count;
         option_index++) {
      const Option argparser_option = default_options[option_index];
      if (flag != argparser_option.short_name)
        continue;
      if (descriptions == NULL)
        return UNDESCRIBED;
      ...
      for (int description_index = 0; description_index < descriptions->count;
        ...
        if (program_name != a_description->name)
          continue;
